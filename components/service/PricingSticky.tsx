        ...
        <div className="border-2    border-zinc-300 rounded-md">
            ...
            <div className="px-6 py-6">
                ...
                <button className='w-full py-4 bg-black rounded flex-col justify-start items-center inline-flex'>
                    <button onClick={() => dispatch(setForm({ userId: data.user_id._id, open: true }))} className="text-center text-white text-base font-bold  hover:text-blue-500">Request to order</button>
                </button>
                <div className="flex justify-center pt-4">

                    <button onClick={() => dispatch(setForm({ userId: data.user_id._id, open: true }))} className="  text-neutral-800 text-sm hover:text-blue-600">Quotation request</button>
                </div>
            </div>
        </div>
    )
